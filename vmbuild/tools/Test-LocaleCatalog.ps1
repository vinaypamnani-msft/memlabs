...
Assert-True -Condition ($phase2Bdc -match 'objectClass=user' -and $phase2Bdc -match 'tokenGroups' -and $phase2Bdc -match "-512\`$" -and $phase2Bdc -notmatch 'primaryGroupToken=512') -What 'BDC verifies the admin effective token by RID 512 instead of searching a localized group name'
Assert-True -Condition ($phase2Bdc -match 'MemLabsLdapNativeJob' -and $phase2Bdc -match 'CreateKillOnClose' -and $phase2Bdc -match 'Assign\(' -and $phase2Bdc -match 'MEMLABS_LDAP_START_GATE' -and $phase2Bdc -match 'WaitForExit\(' -and $phase2Bdc -match '\.Kill\(\)' -and $phase2Bdc -match '-EncodedCommand') -What 'BDC LDAP operations are gated inside a kill-on-close job with a hard wait boundary'
Assert-True -Condition ($phase2Bdc -match 'SearchScope\]::Base' -and $phase2Bdc -match "\[string\[\]\]@\('tokenGroups'\)") -What 'BDC retrieves tokenGroups with a BASE-scope query on the user object'
Assert-True -Condition ($phase2Bdc -match 'Diagnostics\.Stopwatch\]::StartNew' -and $phase2Bdc -match 'while \(`\$timer\.Elapsed\.TotalSeconds -lt' -and $phase2Bdc -match 'Math\]::Min\(15, \[Math\]::Floor') -What 'BDC membership verification uses monotonic elapsed time and clamps its final sleep'
Assert-True -Condition ($phase2Bdc -match 'DateTimeStyles\]::RoundtripKind' -and $phase2Bdc -match 'lastReboot\.ToUniversalTime\(\)') -What 'BDC reboot throttling preserves the UTC marker instant'
Assert-True -Condition ($enableBlm -match 'primaryGroupToken=512') -What 'BLM resolves Domain Admins by RID 512 rather than a localized name'
$utcRebootMarker = [DateTime]::UtcNow.ToString('o')
$parsedUtcRebootMarker = [DateTime]::Parse($utcRebootMarker, [Globalization.CultureInfo]::InvariantCulture, [Globalization.DateTimeStyles]::RoundtripKind)
Assert-Equal -Expected ([DateTimeKind]::Utc) -Actual $parsedUtcRebootMarker.Kind -What 'BDC reboot marker round-trip remains UTC'
Assert-True -Condition ([Math]::Abs(([DateTime]::UtcNow - $parsedUtcRebootMarker.ToUniversalTime()).TotalSeconds) -lt 5) -What 'BDC reboot marker round-trip preserves the recorded instant'
...
$standaloneAdditionsPath = Join-Path $RootPath 'config\tests\Locale-CM-Standalone-Additions.json'
$standaloneAdditions = Get-Content -LiteralPath $standaloneAdditionsPath -Raw | ConvertFrom-Json
$standaloneVms = @($standaloneConfig.virtualMachines) + @($standaloneAdditions.virtualMachines)
Assert-True -Condition ($standaloneRunner -match "ProductType -eq 1\) \{ 'C:\\VirtualMachines' \} else \{ 'E:\\VirtualMachines' \}" -and $standaloneRunner -match "storageRoot -eq 'C:\\'.*?ProductType -ne 1") -What 'standalone locale test defaults to C on Windows Client and E on Windows Server while rejecting explicit C on Server in plan and execution modes'
...
Assert-True -Condition ($standaloneRunner -match 'Select -Stage Core or -Stage Additions when resuming' -and $standaloneRunner -match 'Fresh \$Stage stage refused' -and $standaloneRunner -match '\$Stage stage resume from Phase') -What 'standalone locale test distinguishes all-stage, fresh, and stage-specific resume safety'
...
Assert-Equal -Expected 'DC1=DC,SQL1=DomainMember,PS1SITE=Primary,BDC1=BDC,FS1=FileServer,SRV1=DomainMember,DPMP1=SiteSystem,CL1=DomainMember' -Actual (@($standaloneVms | ForEach-Object { "$($_.vmName)=$($_.role)" }) -join ',') -What 'standalone locale stages cover identity, SQL, ConfigMgr, storage, site-system, and client roles'
Assert-Equal -Expected 'C:\VirtualMachines|C:\VirtualMachines' -Actual "$($standaloneConfig.vmOptions.basePath)|$($standaloneAdditions.vmOptions.basePath)" -What 'standalone locale stages use matching supported Windows Client host storage'
Assert-Equal -Expected 22GB -Actual (@($standaloneConfig.virtualMachines | ForEach-Object { $_.memory / 1 }) | Measure-Object -Sum).Sum -What 'standalone locale core fits the loaded 64 GB host budget'
Assert-Equal -Expected 15GB -Actual (@($standaloneAdditions.virtualMachines | ForEach-Object { $_.memory / 1 }) | Measure-Object -Sum).Sum -What 'standalone locale additions fit after core dynamic-memory settlement'
Assert-Equal -Expected 'BDC1=3GB/1GB,FS1=3GB/1GB,SRV1=2GB/1GB,DPMP1=3GB/1GB,CL1=4GB/1GB' -Actual (@($standaloneAdditions.virtualMachines | ForEach-Object { "$($_.vmName)=$($_.memory)/$($_.dynamicMinRam)" }) -join ',') -What 'standalone locale additions retain the reviewed per-role memory profile'
Assert-True -Condition ($standaloneRunner -match 'Wait-LocaleStageCapacity -RequiredAvailableGB 23' -and $standaloneRunner.Contains("Get-Counter '\Memory\Available MBytes'")) -What 'standalone locale runner gates additions on the validator memory counter plus host reserve'
$standaloneSqlAoProperties = @(foreach ($vm in $standaloneVms) {
...
Assert-Equal -Expected 0 -Actual $standaloneSqlAoProperties.Count -What 'standalone locale stages contain no SQLAO-only properties'
