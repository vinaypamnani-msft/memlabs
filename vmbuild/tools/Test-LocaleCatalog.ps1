...
Assert-Equal -Expected 17GB -Actual (@($standaloneAdditions.virtualMachines | ForEach-Object { $_.memory / 1 }) | Measure-Object -Sum).Sum -What 'standalone locale additions fit after core dynamic-memory settlement'
Assert-True -Condition ($standaloneRunner -match 'Wait-LocaleStageCapacity -RequiredAvailableGB 25' -and $standaloneRunner.Contains("Get-Counter '\Memory\Available MBytes'")) -What 'standalone locale runner gates additions on the validator memory counter plus host reserve'
