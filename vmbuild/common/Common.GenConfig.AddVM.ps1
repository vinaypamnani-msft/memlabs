...
function Resolve-AvailableNewVmName {
    ...
    while ($candidate) {
        ...
        $menuCollision = @($ExistingMachines | Where-Object { $_.vmName -eq $candidate }).Count -gt 0
        $liveCollision = $false
        try { $liveCollision = [bool](Get-VM -Name $candidate -ErrorAction SilentlyContinue) } catch {}
        if (-not $configCollision -and -not $menuCollision -and -not $liveCollision) { return $candidate }

        Write-Log "Autogenerated VM name '$candidate' is already in use (config=$configCollision, menu=$menuCollision, Hyper-V=$liveCollision); trying the next suffix." -LogOnly
...
function Add-NewVMForRole {
    ...
    if ([string]::IsNullOrWhiteSpace($Name)) {
        ...
        $machineName = Resolve-AvailableNewVmName -InitialName $machineName `
            -NamingConfig $oldConfig -TargetConfig $ConfigToModify -VM $virtualMachine `
