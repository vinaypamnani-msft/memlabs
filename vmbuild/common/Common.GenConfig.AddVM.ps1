...
function Resolve-AvailableNewVmName {
    [CmdletBinding()]
    param(
        [string] $InitialName,
        [string] $Domain,
        ...
        [object] $VM,
        [string] $Role,
        [object[]] $ExistingMachines
    )

    $candidate = $InitialName
    $deployedPrefix = [string]$TargetConfig.vmOptions.prefix
    if (-not [string]::IsNullOrWhiteSpace($Domain) -and
        -not [string]::Equals([string]$TargetConfig.vmOptions.domainName, $Domain, [System.StringComparison]::OrdinalIgnoreCase)) {
        $deployedPrefix = get-PrefixForDomain -Domain $Domain
    }
    $attemptedNames = @{}
    while ($candidate) {
        $nameKey = $candidate.ToUpperInvariant()
        if ($attemptedNames.ContainsKey($nameKey)) {
            Write-Log "New VM naming repeated '$candidate' while adding role '$Role'; aborting instead of selecting an existing VM." -Failure
            return $null
        }
        $attemptedNames[$nameKey] = $true

        $configCollision = @($TargetConfig.virtualMachines | Where-Object { $_.vmName -eq $candidate }).Count -gt 0
    $deployedCandidate = $deployedPrefix + $candidate
    $menuCollision = @($ExistingMachines | Where-Object { $_.vmName -eq $deployedCandidate }).Count -gt 0
        $liveCollision = $false
    try { $liveCollision = [bool](Get-VM -Name $deployedCandidate -ErrorAction SilentlyContinue) } catch {}
        if (-not $configCollision -and -not $menuCollision -and -not $liveCollision) { return $candidate }

    Write-Log "Autogenerated VM name '$candidate' (deployed as '$deployedCandidate') is already in use (config=$configCollision, menu=$menuCollision, Hyper-V=$liveCollision); trying the next suffix." -LogOnly
        if ($null -eq $NamingConfig.virtualMachines) { $NamingConfig | Add-Member -MemberType NoteProperty -Name virtualMachines -Value @() -Force }
        if ($Role -in @('CAS', 'Primary', 'Secondary')) {
            $NamingConfig.virtualMachines += [pscustomobject]@{ vmName = $candidate; siteCode = $VM.siteCode }
            $nextSiteCode = Get-NewSiteCode -Domain $Domain -Role $Role -ConfigToCheck $NamingConfig
            if ([string]::IsNullOrWhiteSpace($nextSiteCode)) {
                Write-Log "Could not allocate another site code after generated VM name '$candidate' collided while adding role '$Role'." -Failure
                return $null
            }
            $VM.siteCode = $nextSiteCode
        }
        else {
            $NamingConfig.virtualMachines += [pscustomobject]@{ vmName = $candidate }
        }
        $candidate = Get-NewMachineName -ConfigToCheck $NamingConfig -vm $VM
    }
    return $null
}
...
function Add-NewVMForRole {
    ...
    if ([string]::IsNullOrWhiteSpace($Name)) {
        ...
        # Get-NewMachineName normally checks both config and Get-List, but the
        # latter can be a stale/partial cache. A collision here used to fall
        # through to the final duplicate guard, which skipped the add and then
        # returned that EXISTING name when ReturnMachineName=true. The caller
        # consequently opened the deployed VM's properties and made a failed
        # add look like success. Recheck against every inventory already in the
        # interactive process plus live Hyper-V, reserve collisions in the
        # naming clone, and ask the normal generator for the next suffix.
        $machineName = Resolve-AvailableNewVmName -InitialName $machineName `
            -Domain $Domain -NamingConfig $oldConfig -TargetConfig $ConfigToModify -VM $virtualMachine `
            -Role $role -ExistingMachines @($global:existingMachines)
        if (-not $machineName) { return }
    ...
    # domain itself), StandaloneRootCA (offline, no networking), and OSDClient
    # (no installed guest in which to configure a proxy before OOBE). Both
    # Windows and Linux VMs are otherwise eligible -- Windows clients are configured
    ...
    $proxyExcluded = @('Proxy', 'DC', 'BDC', 'StandaloneRootCA', 'OSDClient')
    ...
    if ($existingDPMP -eq 0) {
        if ($virtualMachine.siteCode) {
            $newSiteCode = $virtualMachine.siteCode
        }
        elseif (-not $newSiteCode) {
