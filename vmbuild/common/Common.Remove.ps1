    ...
    if ($vmTest) {
        ...
        foreach ($adapter in $adapters) {
            if ($adapter.SwitchName -eq "cluster") {
                try {
                    Write-Log -LogOnly '9Calling Remove-DhcpServerv4Reservation  -ScopeId 10.250.250.0 -ClientId $adapter.MacAddress -ErrorAction SilentlyContinue -WhatIf:$WhatIf'
                ...
                Write-Log -LogOnly '10Calling Remove-DhcpServerv4Reservation  -ScopeId $vmFromList.Network -ClientId $adapter.MacAddress -ErrorAction SilentlyContinue -WhatIf:$WhatIf'
