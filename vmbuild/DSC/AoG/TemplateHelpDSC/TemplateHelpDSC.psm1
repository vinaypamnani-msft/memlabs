...
class ChangeSQLServicesAccount {
	...
	[void] Set()
	{
		...
		$serviceName = if ($_SQLInstanceName -eq "MSSQLSERVER") { $_SQLInstanceName }
		else { "MSSQL`$$_SQLInstanceName" }
		...
		if ($services.State -eq 'Running')
		{
			...
			$sqlAgentService = if ($_SQLInstanceName -eq "SQLSERVERAGENT") { $_SQLInstanceName }
			else { "SQLAgent`$$_SQLInstanceName" }
