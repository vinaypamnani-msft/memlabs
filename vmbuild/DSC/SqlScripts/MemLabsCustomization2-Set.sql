...
Date: Jan 15, 2025
Purpose: This script changes the compatibility levels of all CM_xxx database to 150 if they are greater than 150.
*/
...
SELECT @level =
  MAX(compatibility_level) FROM sys.databases
...
SELECT name
FROM sys.databases
WHERE (name LIKE 'CM[_]___') -- Only match CM_XXX databases
AND is_read_only = 0
...
    SET @sql = N'ALTER DATABASE ' + QUOTENAME(@dbname) + N' SET COMPATIBILITY_LEVEL = 150'    --example CM_PS1.sys.sp_executesql
    
    EXEC @exec @sql
  END TRY
      BEGIN CATCH
