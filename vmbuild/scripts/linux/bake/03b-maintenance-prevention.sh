...
# Ubuntu cloud images carry additional /etc/default/grub.d/*.cfg snippets.
# update-grub sources those AFTER /etc/default/grub, so editing only the main
# file can look correct here while a cloud-image snippet silently replaces the
# value used to generate /boot/grub/grub.cfg. Write the final effective values
# into a lexically-last drop-in. This is deliberately an exact assignment, not
# an append: reruns and future upstream defaults must not restore a second
# unbounded console=ttyS0 token after the 115200 console.
mkdir -p /etc/default/grub.d
cat > /etc/default/grub.d/99-memlabs.cfg << EOF
# Generated by MemLabs. Loaded after Ubuntu cloud-image GRUB defaults.
GRUB_CMDLINE_LINUX_DEFAULT="$(_grub_get GRUB_CMDLINE_LINUX_DEFAULT)"
GRUB_CMDLINE_LINUX="$(_grub_get GRUB_CMDLINE_LINUX)"
GRUB_TIMEOUT=1
GRUB_RECORDFAIL_TIMEOUT=1
EOF
chmod 0644 /etc/default/grub.d/99-memlabs.cfg
echo "grub: wrote final override /etc/default/grub.d/99-memlabs.cfg"
update-grub
...
if [ ! -f /boot/grub/grub.cfg ] || ! grep -q 'console=ttyS0,115200n8' /boot/grub/grub.cfg; then
    echo "ERROR: update-grub did not render console=ttyS0,115200n8 into /boot/grub/grub.cfg" >&2
    echo "GRUB defaults and snippets:" >&2
    grep -H -E '^GRUB_CMDLINE_LINUX(_DEFAULT)?=|^GRUB_TIMEOUT|^GRUB_RECORDFAIL' \
        /etc/default/grub /etc/default/grub.d/*.cfg 2>/dev/null >&2 || true
    exit 1
fi
echo "grub: generated /boot/grub/grub.cfg contains console=ttyS0,115200n8"
